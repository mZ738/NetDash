...
class NetDashCard extends LitElement {
  ...
  static getConfigElement() {
    return document.createElement('netdash-editor');
  }
  ...
  static get styles() {
    return css`
      ...
      .nd-search-input { width:100%; background:var(--nd-card); border:1px solid var(--nd-border);
        ...
        font-family:inherit; font-size:13px; outline:none; transition:border-color .2s,box-shadow .2s; box-sizing: border-box; }
  ...
  */
  ...
  _vc(id) { 
    const v = (this._config.vlans || []).find(x => x.id === id); 
  ...
  _cp(ip, e) {
    ...
    if (navigator.clipboard && window.isSecureContext) {
      ...
      const t = document.createElement('textarea'); t.value = ip; 
      ...
      <div class="nd-hdr">
        ...
        ${ss ? html`
          <div class="nd-search-box">
            ...
            <input type="text" class="nd-search-input" placeholder="Search devices, IPs, types..." @input=${this._onS}>
          </div>` : nothing}
      </div>`;
  }
    ...
    return html`
      <div class="nd-filters">
        <button class="nd-ftab ${this._filter === 'all' ? 'on' : ''}" @click=${() => this._onF('all')}>
          All<span class="nd-fcnt">${ds.length}</span>
        </button>
        ${vs.map(v => html`
          <button class="nd-ftab ${this._filter === v.id ? 'on' : ''}" @click=${() => this._onF(v.id)}>
            <span class="nd-fdot" style="background:${v.color}"></span>${v.label}
            <span class="nd-fcnt">${ds.filter(d => d.vlan === v.id).length}</span>
          </button>`)}
      </div>`;
  }
  ...
  _rStat() {
    ...
    let h = 0, ht = 0; 
    ds.forEach(d => { (d.url || '').startsWith('https') ? h++ : ht++; });
    return html`
      <div class="nd-stats">
        ...
        <div class="nd-st"><ha-icon icon="mdi:layers-outline" style="color:var(--nd-rose)"></ha-icon>
          <div><div class="nd-st-v">${vs.length}</div><div class="nd-st-l">VLANs</div></div></div>
      </div>`;
  }
  ...
  _rDev(d, i) {
    ...
    return html`
      <div class="nd-dev" style="--nd-accent:${vc};animation-delay:${i * 20}ms"
        @click=${() => this._open(d.url)} tabindex="0" role="link" aria-label="Open ${d.name}">
        <div class="nd-dicon" style="background:${this._h2r(co, .12)};color:${co}">
        ...
        <div class="nd-dinfo">
          ...
          <div class="nd-badges">
            ...
            <span class="nd-badge ${pc}">${pr}</span>
          </div>
          ${d.ip ? html`
            <div class="nd-ip-row">
              <span class="nd-ip" @click=${e => this._cp(d.ip, e)} title="Click to copy">${d.ip}</span>
              <ha-icon icon="mdi:content-copy" class="nd-cpbtn" @click=${e => this._cp(d.ip, e)}></ha-icon>
            </div>` : nothing}
  ...
  _rVlan(g) {
    return html`
      <div class="nd-vlan">
        ...
        <div class="nd-grid">${g.devices.map((d, i) => this._rDev(d, i))}</div>
      </div>`;
  }
  ...
  render() {
    ...
    return html`
      <div class="nd-wrap">
        ${this._rHdr()}${this._rFil()}${this._rStat()}
        ${g.length > 0 ? g.map(x => this._rVlan(x)) : this._rEmpty()}
      </div>`;
  }
}
...
class NetDashEditor extends LitElement {
  ...
  static get styles() {
    return css`
      ...
      .ne-sec-cnt { font-size:11px; color:var(--secondary-text-color); font-family:'Roboto Mono',monospace; }
  ...
  _togV(i, exp) { this._openV = exp ? [...this._openV, i] : this._openV.filter(x => x !== i); }
  _togD(i, exp) { this._openD = exp ? [...this._openD, i] : this._openD.filter(x => x !== i); }
  ...
  _filteredDevs() {
    const ds = this._config.devices || [], q = this._dSearch.toLowerCase().trim();
    return ds.filter(d => {
      ...
      <div class="ne-sec">
        ...
        <div class="ne-fields">
          ...
          <div class="ne-toggles">
            <ha-form-row>
              <ha-switch .checked=${c.show_search !== false} @change=${e => this._setSS(e.target.checked)}></ha-switch>
              <span slot="label">Show search bar</span>
            </ha-form-row>
            <ha-form-row>
              <ha-switch .checked=${c.show_filters !== false} @change=${e => this._setSF(e.target.checked)}></ha-switch>
              <span slot="label">Show VLAN filter tabs</span>
            </ha-form-row>
            <ha-form-row>
              <ha-switch .checked=${c.show_stats !== false} @change=${e => this._setSt(e.target.checked)}></ha-switch>
